...
def push_after_process_task(self, prev_task_result):
    ...
    task_results = asyncio.run(
        _index_data_content(self, project_id=project_id, do_rest=do_reset)
    )
...
def process_and_push_workflow(  self, project_id: int, 
                                ...
                                overlap_size: int, do_reset: int,file_type:str):

    workflow = chain(
        process_project_files.s(project_id=project_id, file_id=file_id, chunk_size=chunk_size
                                , overlap_size=overlap_size, do_reset=do_reset,file_type=file_type),
